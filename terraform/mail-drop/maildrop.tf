...
resource "proxmox_virtual_environment_vm" "mail_drop_server" {
  node_name = "proxmox"
  name      = "mail-drop-server"
  started   = true
...
resource "null_resource" "docker_setup_and_run" {
  ...
  depends_on = [proxmox_virtual_environment_vm.mail_drop_server]
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "mkdir -p /home/bence/mail-drop",
    ]
  }

  # fájl feltöltése a VM-re, a file provisioner teszi lehetővé, hogy fájlt töltünk fel az installáció után
  provisioner "file" {
    source      = "../../mail-drop/docker-compose.yaml"
    destination = "/home/bence/docker-compose.yaml"
  }

  provisioner "file" {
    source      = "../../mail-drop/config.json"
    destination = "/home/bence/config.json"
  }

  # Docker konténerek indítása a feltöltött docker-compose fájl alapján
  provisioner "remote-exec" {
  inline = [
    "mv /home/bence/docker-compose.yaml /home/bence/mail-drop/docker-compose.yaml",
    "mv /home/bence/config.json /home/bence/mail-drop/config.json",
    "cd /home/bence/mail-drop",
